...
def comp_radar_fit(
    ...
    vars = ['max_value','count_cells'] # vars to extract from the dataset
    if cov is not None:
        vars.extend(cov)
    # normal stuff
    ...
    ds = dataset[vars].isel(quantv=rand_index).\
...
if __name__ == "__main__":
    ...
    parser.add_argument('--covariate', type=str, default=None) # Name of covariate to use. If none will be temperature
    ...
    if args.covariate is None:
        ...
        cov_var = args.covariate
    outfile = f'gev_fit_{cov_var}.nc'
    summary_file = f'gev_summary_{cov_var}.txt'
    ...
    output_fit_cov = output_dir / outfile
    output_fit = output_dir / "gev_fit.nc"
    output_summary = output_dir / summary_file
    output_fit_cov_bs = output_dir / (output_fit_cov.stem + '_bs.nc')
    output_fit_bs = output_dir / "gev_fit_bs.nc"
    files = [output_fit, output_fit_cov, output_summary]
    if args.bootstrap_samples > 0:
        my_logger.info(f"Bootstrapping")
        files.extend([output_fit_cov_bs, output_fit_bs])
    # Modify the output files if time_range is set.
    if time_range is not None:
        files = [f.with_name(f"{f.stem}{out_post}{f.suffix}") for f in files]
        output_fit, output_fit_cov, output_summary = tuple(files[0:3])
        if args.bootstrap_samples > 0:
            output_fit_cov_bs, output_fit_bs = tuple(files[3:5])
    ...
    radar_dataset = xarray.open_dataset(args.input_file, chunks=dict(EventTime=-1, resample_prd=1, quantv=-1),
                                        cache=True)  # load the processed radar
    if time_range is not None:
    ...
    for v in radar_dataset.data_vars:
        if set(msk.dims) == set(radar_dataset[v].dims):
    ...
    my_logger.info(f"Output files: fit_t: {output_fit_cov}, fit: {output_fit} & summary: {output_summary}")
    ...
    dd = xarray.concat([xarray.DataArray(0.0)] * 2, dim='parameter').assign_coords(
        parameter=['Dlocation_Tanom', 'Dscale_Tanom'])
    ...
    fit_cov, fit_cov_bs = comp_radar_fit(radar_dataset, cov=[cov_var + 'anom'],
                                     n_samples=args.nsamples, bootstrap_samples=args.bootstrap_samples,
                                     extra_attrs=extra_attrs, name=f'fit_{cov_var}', file=output_fit_cov,
                                     bootstrap_file=output_fit_cov_bs,
    ...
    with output_summary.open('w') as f:
        # print out the AIC
        q_aic = (fit_cov.AIC - fit.AIC).quantile([0.1, 0.5, 0.9], dim='sample').to_dataframe().unstack()
        print(f"AIC: {q_aic.round(-1)}", file=f)
        if args.bootstrap_samples > 0:
            q_aic_bs = (fit_cov_bs.AIC - fit_bs.AIC).quantile([0.1, 0.5, 0.9],
                                                            dim='bootstrap_sample').to_dataframe().unstack()
        ...
        for p in ['location', 'scale']:
            ...
            if args.bootstrap_samples > 0:
                dfract_bs = fit_cov_bs.Parameters.sel(parameter=dp) / fit_cov_bs.Parameters.sel(parameter=p)
                q_bs = dfract_bs.quantile([0.1, 0.5, 0.9], dim='bootstrap_sample').to_dataframe().unstack()
                print(f"BS fract -cov  {dp}: {q_bs.round(3)}", file=f)
    my_logger.debug(f'Wrote summary file {output_summary}')
